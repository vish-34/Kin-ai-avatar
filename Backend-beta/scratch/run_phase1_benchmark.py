...
def run_phase1_benchmark():
    print("Initializing PersonaCloneEngine for Phase 1 Benchmark...")
    engine = PersonaCloneEngine()
    print(f"Engine ready. Model: {engine.model} (max_tokens={engine.max_tokens})")

    # Identical 57 test cases from the baseline audit
    test_cases = [
    ...
    out_file = BACKEND_DIR / "scratch" / "phase1_benchmark_results.json"
    results_map = {}
    if out_file.exists():
        try:
    ...
    print(f"Resuming benchmark: {len(results_map)} valid tests cached, {len(test_cases) - len(results_map)} tests to run.")
    ...
    for tc in test_cases:
        ...
        for turn_idx, user_msg in enumerate(tc["turns"], 1):
            ...
            time.sleep(1.5)  # Pace queries to respect TPM
    ...
    print(f"\nPhase 1 Benchmark completed! Total valid test cases: {len(results_map)}")
